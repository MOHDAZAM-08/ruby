...
import ExchangeLearn from './ExchangeLearn';
import LiquidityLearn from './LiquidityLearn';
import MigrationLearn from './MigrationLearn';
  ...
  return (
    <>
      ...
      <div className='flex items-start justify-around p-8 rounded  '>
        ...
        <div className='z-[0] h-[60vh] w-[20%] rounded-[10px] p-5 flex flex-col justify-evenly items-start bg-[rgba(77,77,77,0.05)] shadow-custom-dual backdrop-blur-[8.5px] border border-[rgba(255,255,255,0.09)]'>
          ...
          <div className={`flex items-center justify-between h-10 w-[180px] px-4 rounded cursor-pointer ${activeComponent === 'migration' ? 'border-l-8 border-blue-500 bg-gray-700' : 'hover:border-l-8 hover:border-blue-500 hover:bg-gray-700'}`} onClick={() => handleClick('migration')}>
          LP Migration <IoIosArrowRoundForward />
          </div>
        ...
        {activeComponent === 'migration' && <Migration />}
        </div>

        <div className='h-[70vh] w-56 flex flex-col gap-5'>
        {activeComponent === 'exchange' && <ExchangeLearn/>}
        {activeComponent === 'liquidity' && <LiquidityLearn/>}
        {activeComponent === 'migration' && <MigrationLearn />}
        </div>
